...
dataExcel='MWLineShape.xlsx';dataSheet=secName
...
autoSetYAxisFormatnum=True    #是否自动设置y轴数字精度
...
for i in range(0,nPoints):
    ...
    for j in range(0,dataCounts):        
        temp.append(sh.cell_value(startRow+j,i+1)*reviseFactor)
...
ax.set_xlabel('跨号',fontsize=labelSize)
ax.set_ylabel('相对高程（mm）',fontsize=labelSize)
